...
- **功能亮点**：按章节刷题 · 收藏题目 · 刷题历史记录 · 错题重做 · 二维码扫码即用
...
首页标题下方设「📱 生成二维码」按钮，使用 `api.qrserver.com` 免费 API 生成**整页二维码**：

```javascript
function showPageQR() {
  const hostUrl = document.getElementById('baseUrl').value.trim() || loadHostUrl();
  if (!hostUrl) { alert('请先填写网页线上地址'); return; }
  saveHostUrl();
...
扫码后进入首页，用户输入激活码解锁后看到全部章节列表，而非直接进入某一章节。
...
| 要点 | 方案 |
|------|------|
| 题库来源 | .doc → win32com 转 .txt → Python 解析 → JSON；或 PDF → PyPDF2 提取 → 解析 |
| 选项处理 | 支持逐行解析 + 内联拆分（`A.xxx B.xxx` 格式），确保选项不合并 |
| 答案匹配 | 支持嵌入答案（`正确答案：X`）或独立答案文件匹配 |
| 前端框架 | 纯 HTML/CSS/JS，无依赖 |
| 激活码验证 | SHA-256 哈希比对，离线可用 |
| 题库存储 | 嵌入 HTML 的 JS 对象 |
| 托管 | GitHub Pages（免费） |
| 二维码 | api.qrserver.com 免费 API，整页二维码（扫码进入首页看全部章节） |
| 题型 | 单选/多选/判断/计算 四种 |
| 出题顺序 | 保持原始章节顺序，可手动打乱 |
...
| **设计风格** | **暖色调统一标准，见下方 §5.1** |

### 5.1 页面设计统一标准

后续所有刷题网页均使用以下暖色调设计规范（以「期末成本会计刷题酱」为模板）：
...
| 次要文字 | `#8b7355` | 暖灰棕 |
| 边框 | `#e5d9cc` | 浅暖灰 |
| 强调色 | `#b8860b` | 暗金色（按钮、选中态） |
| 强调浅色 | `#fef3c7` | 淡金（标签背景） |
| 正确色 | `#2d8a56` | 绿色 |
| 错误色 | `#c0392b` | 红色 |

**布局规范**
- 移动优先响应式，最大宽度 `680px` 居中
- 卡片圆角 `12px`，按钮/选项圆角 `10px`
- 字体：`"PingFang SC", "Hiragino Sans GB", "Microsoft YaHei", sans-serif`
- 阴影：`0 1px 3px rgba(44,36,22,.06)`

**交互规范**
- 单选题：点击选项 → 立即显示对错 → 正确 800ms 后自动前进
- 多选题：点击多选 → 点「确认答案」→ 显示对错
- 判断题：点击「正确」或「错误」按钮 → 显示对错

**功能标配**
- 激活码锁屏（12 位，XXXX-XXXX-XXXX 格式，SHA-256 验证）
- 章节卡片列表（章节名 + 题数 + 开始刷题按钮）
- ☆ 收藏按钮（刷题顶部栏），首页折叠区查看/跳转
- 刷题历史记录（首页折叠区，按时间倒序，≥80%绿/60-79%橙/<60%红）
- 结果页：大号正确率 + 统计卡片 + 错题回顾 + 重做错题
- 首页「📱 生成二维码」按钮，生成整页二维码（扫码→激活→全部章节列表），非单章二维码
...
**Q: 二维码扫不出来怎么办？**
生成的是整页二维码（指向首页 URL），URL 简短（约 40-60 字符），扫码识别率高。
